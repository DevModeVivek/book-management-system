package com.vivek.bookservice.repository;
...
public interface IBookRepository extends BaseRepository<Book> {
    ...
    @Query("SELECT CASE WHEN COUNT(b) > 0 THEN true ELSE false END FROM Book b " +
           "WHERE b.isbn = :isbn AND b.isActive = true")
    boolean existsByIsbnAndIsActiveTrue(@Param("isbn") String isbn);
    
    @Query("SELECT CASE WHEN COUNT(b) > 0 THEN true ELSE false END FROM Book b " +
           "WHERE b.isbn = :isbn AND b.id != :excludeId AND b.isActive = true")
    ...
    @Query("SELECT b FROM Book b WHERE LOWER(b.title) = LOWER(:title) AND b.isActive = true")
    List<Book> findByTitleIgnoreCaseAndIsActiveTrue(@Param("title") String title);
    
    @Query("SELECT b FROM Book b WHERE LOWER(b.author) = LOWER(:author) AND b.isActive = true ORDER BY b.title ASC")
    List<Book> findByAuthorIgnoreCaseAndIsActiveTrue(@Param("author") String author);
    
    @Query("SELECT b FROM Book b WHERE LOWER(b.genre) = LOWER(:genre) AND b.isActive = true ORDER BY b.title ASC")
    List<Book> findByGenreIgnoreCaseAndIsActiveTrue(@Param("genre") String genre);
    
    @Query("SELECT b FROM Book b WHERE LOWER(b.publisher) = LOWER(:publisher) AND b.isActive = true ORDER BY b.title ASC")
    List<Book> findByPublisherIgnoreCaseAndIsActiveTrue(@Param("publisher") String publisher);
    
    @Query("SELECT b FROM Book b WHERE LOWER(b.language) = LOWER(:language) AND b.isActive = true ORDER BY b.title ASC")
    List<Book> findByLanguageIgnoreCaseAndIsActiveTrue(@Param("language") String language);
    
    // ============= DATE-BASED OPERATIONS =============
    
    @Query("SELECT b FROM Book b WHERE b.publishedDate > :date AND b.isActive = true ORDER BY b.publishedDate DESC")
    ...
    @Query("SELECT b FROM Book b WHERE b.publishedDate BETWEEN :startDate AND :endDate AND b.isActive = true ORDER BY b.publishedDate DESC")
    List<Book> findByPublishedDateBetweenAndIsActiveTrue(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);
    
    @Query("SELECT b FROM Book b WHERE YEAR(b.publishedDate) = :year AND b.isActive = true ORDER BY b.publishedDate DESC")
    List<Book> findByPublishedYearAndIsActiveTrue(@Param("year") int year);
    
    // ============= PRICE-BASED OPERATIONS =============
    
    @Query("SELECT b FROM Book b WHERE b.price IS NOT NULL AND b.price > :minPrice AND b.isActive = true ORDER BY b.price ASC")
    List<Book> findByPriceGreaterThanAndIsActiveTrue(@Param("minPrice") BigDecimal minPrice);
    
    @Query("SELECT b FROM Book b WHERE b.price IS NOT NULL AND b.price < :maxPrice AND b.isActive = true ORDER BY b.price ASC")
    List<Book> findByPriceLessThanAndIsActiveTrue(@Param("maxPrice") BigDecimal maxPrice);
    
    @Query("SELECT b FROM Book b WHERE b.price IS NOT NULL AND b.price BETWEEN :minPrice AND :maxPrice AND b.isActive = true ORDER BY b.price ASC")
    List<Book> findByPriceBetweenAndIsActiveTrue(@Param("minPrice") BigDecimal minPrice, @Param("maxPrice") BigDecimal maxPrice);
    
    @Query("SELECT b FROM Book b WHERE b.price IS NULL AND b.isActive = true ORDER BY b.title ASC")
    List<Book> findByPriceIsNullAndIsActiveTrue();
    
    // ============= PAGE COUNT OPERATIONS =============
    
    @Query("SELECT b FROM Book b WHERE b.pageCount IS NOT NULL AND b.pageCount > :minPages AND b.isActive = true ORDER BY b.pageCount ASC")
    List<Book> findByPageCountGreaterThanAndIsActiveTrue(@Param("minPages") Integer minPages);
    
    @Query("SELECT b FROM Book b WHERE b.pageCount IS NOT NULL AND b.pageCount < :maxPages AND b.isActive = true ORDER BY b.pageCount ASC")
    List<Book> findByPageCountLessThanAndIsActiveTrue(@Param("maxPages") Integer maxPages);
    
    @Query("SELECT b FROM Book b WHERE b.pageCount IS NOT NULL AND b.pageCount BETWEEN :minPages AND :maxPages AND b.isActive = true ORDER BY b.pageCount ASC")
    ...
    @Query("SELECT b FROM Book b WHERE LOWER(b.author) = LOWER(:author) AND LOWER(b.genre) = LOWER(:genre) AND b.isActive = true ORDER BY b.title ASC")
    List<Book> findByAuthorAndGenreIgnoreCaseAndIsActiveTrue(@Param("author") String author, @Param("genre") String genre);
    
    @Query("SELECT b FROM Book b WHERE LOWER(b.genre) = LOWER(:genre) AND LOWER(b.publisher) = LOWER(:publisher) AND b.isActive = true ORDER BY b.title ASC")
    List<Book> findByGenreAndPublisherIgnoreCaseAndIsActiveTrue(@Param("genre") String genre, @Param("publisher") String publisher);
    
    @Query("SELECT b FROM Book b WHERE LOWER(b.genre) = LOWER(:genre) AND b.publishedDate BETWEEN :startDate AND :endDate AND b.isActive = true ORDER BY b.publishedDate DESC")
    ...
    @Query("SELECT COUNT(b) FROM Book b WHERE LOWER(b.genre) = LOWER(:genre) AND b.isActive = true")
    long countByGenreIgnoreCaseAndIsActiveTrue(@Param("genre") String genre);
    
    @Query("SELECT COUNT(b) FROM Book b WHERE LOWER(b.author) = LOWER(:author) AND b.isActive = true")
    long countByAuthorIgnoreCaseAndIsActiveTrue(@Param("author") String author);
    
    @Query("SELECT COUNT(b) FROM Book b WHERE LOWER(b.publisher) = LOWER(:publisher) AND b.isActive = true")
    long countByPublisherIgnoreCaseAndIsActiveTrue(@Param("publisher") String publisher);
    
    @Query("SELECT COUNT(b) FROM Book b WHERE b.publishedDate > :date AND b.isActive = true")
    long countByPublishedDateAfterAndIsActiveTrue(@Param("date") LocalDate date);
    
    @Query("SELECT COUNT(b) FROM Book b WHERE b.price IS NOT NULL AND b.price BETWEEN :minPrice AND :maxPrice AND b.isActive = true")
    long countByPriceBetweenAndIsActiveTrue(@Param("minPrice") BigDecimal minPrice, @Param("maxPrice") BigDecimal maxPrice);
    
    // ============= ADVANCED ANALYTICS OPERATIONS =============
    
    @Query("SELECT b.genre, COUNT(b) FROM Book b WHERE b.isActive = true GROUP BY b.genre ORDER BY COUNT(b) DESC")
    List<Object[]> findGenreStatistics();
    
    @Query("SELECT b.author, COUNT(b) FROM Book b WHERE b.isActive = true GROUP BY b.author ORDER BY COUNT(b) DESC")
    List<Object[]> findAuthorStatistics();
    
    @Query("SELECT b.publisher, COUNT(b) FROM Book b WHERE b.isActive = true GROUP BY b.publisher ORDER BY COUNT(b) DESC")
    List<Object[]> findPublisherStatistics();
    
    @Query("SELECT YEAR(b.publishedDate) as year, COUNT(b) FROM Book b WHERE b.isActive = true GROUP BY YEAR(b.publishedDate) ORDER BY year DESC")
    List<Object[]> findPublicationYearStatistics();
    
    @Query("SELECT AVG(b.price) FROM Book b WHERE b.price IS NOT NULL AND b.isActive = true")
    BigDecimal findAveragePrice();
    
    @Query("SELECT MIN(b.price) FROM Book b WHERE b.price IS NOT NULL AND b.isActive = true")
    BigDecimal findMinPrice();
    
    @Query("SELECT MAX(b.price) FROM Book b WHERE b.price IS NOT NULL AND b.isActive = true")
    BigDecimal findMaxPrice();
    
    @Query("SELECT AVG(b.pageCount) FROM Book b WHERE b.pageCount IS NOT NULL AND b.isActive = true")
    ...
    @Query("SELECT b FROM Book b WHERE LOWER(b.genre) = LOWER(:genre) AND b.isActive = true ORDER BY b.publishedDate DESC")
    List<Book> findBooksByGenreOrderByNewest(@Param("genre") String genre, Pageable pageable);
    
    @Query("SELECT b FROM Book b WHERE LOWER(b.author) = LOWER(:author) AND b.isActive = true ORDER BY b.publishedDate DESC")
